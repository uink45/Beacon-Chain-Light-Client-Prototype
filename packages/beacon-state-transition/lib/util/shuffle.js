...
exports.unshuffleList = exports.shuffleList = void 0;
/**
 * @module util/objects
 */
...
function shuffleList(input, seed) {
...
function unshuffleList(input, seed) {
    innerShuffleList(input, seed, false);
}
exports.unshuffleList = unshuffleList;
...
function innerShuffleList(input, seed, dir) {
    ...
    if (lodestar_params_1.SHUFFLE_ROUND_COUNT == 0) {
        // no shuffling
        return;
    }
    ...
    lodestar_utils_1.assert.equal(listSize, input.length, "input length does not fit uint32");
    ...
    if (!dir) {
        ...
        r = lodestar_params_1.SHUFFLE_ROUND_COUNT - 1;
    }
    ...
    while (true) {
        ...
        const h = (0, ssz_1.hash)(buf.slice(0, _SHUFFLE_H_PIVOT_VIEW_SIZE));
        const pivot = Number((0, lodestar_utils_1.bytesToBigInt)(h.slice(0, 8)) % BigInt(listSize)) >>> 0;
        ...
        if (dir) {
            // -> shuffle
            r += 1;
            if (r == lodestar_params_1.SHUFFLE_ROUND_COUNT) {
                break;
            }
        }
        else {
