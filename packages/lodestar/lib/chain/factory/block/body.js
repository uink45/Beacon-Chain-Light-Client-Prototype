...
exports.assembleBody = void 0;
const lodestar_types_1 = require("@chainsafe/lodestar-types");
const lodestar_beacon_state_transition_1 = require("@chainsafe/lodestar-beacon-state-transition");
const constants_1 = require("../../../constants");
async function assembleBody(chain, currentState, { randaoReveal, graffiti, blockSlot, parentSlot, parentBlockRoot, feeRecipient, }) {
    // TODO:
    ...
    const blockEpoch = (0, lodestar_beacon_state_transition_1.computeEpochAtSlot)(blockSlot);
    if (blockEpoch >= chain.config.ALTAIR_FORK_EPOCH) {
        blockBody.syncAggregate = chain.syncContributionAndProofPool.getAggregate(parentSlot, parentBlockRoot);
    }
    if (blockEpoch >= chain.config.BELLATRIX_FORK_EPOCH) {
        ...
        const payloadId = await prepareExecutionPayload(chain, finalizedBlockHash !== null && finalizedBlockHash !== void 0 ? finalizedBlockHash : constants_1.ZERO_HASH_HEX, currentState, feeRecipient);
        if (payloadId === null) {
            // Pre-merge, propose a pre-merge block with empty execution and keep the chain going
            blockBody.executionPayload = lodestar_types_1.ssz.bellatrix.ExecutionPayload.defaultValue();
        }
        else {
            blockBody.executionPayload = await chain.executionEngine.getPayload(payloadId);
        }
    }
    return blockBody;
}
exports.assembleBody = assembleBody;
/**
 ...
 */
async function prepareExecutionPayload(chain, finalizedBlockHash, state, suggestedFeeRecipient) {
    ...
    let parentHash;
    if (!lodestar_beacon_state_transition_1.bellatrix.isMergeTransitionComplete(state)) {
        if (!lodestar_types_1.ssz.Root.equals(chain.config.TERMINAL_BLOCK_HASH, constants_1.ZERO_HASH) &&
            (0, lodestar_beacon_state_transition_1.getCurrentEpoch)(state) < chain.config.TERMINAL_BLOCK_HASH_ACTIVATION_EPOCH)
            throw new Error(`InvalidMergeTBH epoch: expected >= ${chain.config.TERMINAL_BLOCK_HASH_ACTIVATION_EPOCH}, actual: ${(0, lodestar_beacon_state_transition_1.getCurrentEpoch)(state)}`);
    ...
    if (!payloadId)
        throw new Error("InvalidPayloadId: Null");
    return payloadId;
}
