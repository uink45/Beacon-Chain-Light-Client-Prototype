...
export declare enum GossipType {
...
export declare enum GossipEncoding {
...
export declare type GossipTopicTypeMap = {
...
export declare type GossipTopicMap = {
    [K in keyof GossipTopicTypeMap]: GossipTopicTypeMap[K] & IGossipTopic;
};
/**
 * Gossip topic split into a struct
 */
export declare type GossipTopic = GossipTopicMap[keyof GossipTopicMap];
export declare type GossipTypeMap = {
...
export declare type GossipFnByType = {
    ...
    [GossipType.sync_committee_contribution_and_proof]: (signedContributionAndProof: altair.SignedContributionAndProof) => Promise<void> | void;
    [GossipType.sync_committee]: (syncCommittee: altair.SyncCommitteeMessage) => Promise<void> | void;
};
export declare type GossipFn = GossipFnByType[keyof GossipFnByType];
...
export declare type GossipEventEmitter = StrictEventEmitter<EventEmitter, IGossipEvents>;
...
export declare type GossipJobQueues = {
    [K in GossipType]: JobItemQueue<[GossipTopic, InMessage, number], void>;
};
export declare type GossipHandlerFn = (object: GossipTypeMap[GossipType], topic: GossipTopicMap[GossipType], peerIdStr: string, seenTimestampSec: number) => Promise<void>;
export declare type GossipHandlers = {
    [K in GossipType]: (object: GossipTypeMap[K], topic: GossipTopicMap[K], peerIdStr: string, seenTimestampSec: number) => Promise<void>;
};
export declare type InMessageTimestamp = InMessage & {
