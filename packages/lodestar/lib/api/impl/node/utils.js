...
exports.formatNodePeer = formatNodePeer;
/**
 ...
 */
function getRevelantConnection(connections) {
    const byStatus = new Map();
    for (const conn of connections) {
        if (conn.stat.status === "open")
            return conn;
        if (!byStatus.has(conn.stat.status))
            byStatus.set(conn.stat.status, conn);
    }
