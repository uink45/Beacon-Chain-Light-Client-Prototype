...
namespace Nethermind.Merkleization
{
    ...
    public static partial class Merkle
    {
        ...
        {
            Merkleizer merkleizer = new Merkleizer(2);
            Root parentRoot = new Root(((System.ReadOnlySpan<byte>)container.ParentRoot));
            Root stateRoot = new Root(((System.ReadOnlySpan<byte>)container.StateRoot));
            Root bodyRoot = new Root(((System.ReadOnlySpan<byte>)container.BodyRoot));
            merkleizer.Feed(container.Slot);
            merkleizer.Feed(parentRoot);
            merkleizer.Feed(stateRoot);
            merkleizer.Feed(bodyRoot);
            merkleizer.CalculateRoot(out root);
        }
