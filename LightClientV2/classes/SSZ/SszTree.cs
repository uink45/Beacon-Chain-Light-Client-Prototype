...
namespace LightClientV2
{
    public class SszTree
    {
        ...
        public ReadOnlySpan<byte> GetChunks()
        {
            var context = new Stack<int>(new[] { 0 });
            try
            {
                return GetChunksRecursive(RootElement, context);
            }
            catch (Exception ex)
            {
        ...
        private ReadOnlySpan<byte> GetChunksRecursive(SszElement element , Stack<int> context)
        {
            switch (element)
            {
                ...
                case SszVector vector:
                    {
                        ...
                        using (var memory = new MemoryStream())
                        {
                            ...
                            {
                                context.Push(index++);
                                memory.Write(GetChunksRecursive(value, context));
                                context.Pop();
                            }
        ...
        public ReadOnlySpan<byte> MerkleizeRecursive(int height, ReadOnlySpan<byte> chunks)
        {
            if (height == 0)
            {
                return chunks;
        ...
        public ReadOnlySpan<byte> Pack(ReadOnlySpan<byte> value)
        {
