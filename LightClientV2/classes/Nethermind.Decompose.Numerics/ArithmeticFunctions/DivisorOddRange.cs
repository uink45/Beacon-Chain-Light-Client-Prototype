...
{
    public class DivisorOddRange : IArithmeticRange<ushort, ulong>
    {
        private class Data
        {
        ...
        private ulong FinishBlock(long k0, int length, long[] products, ushort[] values, long kmin, ulong[] sums, long smin, ulong sum0, bool onlySums)
        {
            ...
            if (onlySums)
            {
                ...
                {
                    sums[k - deltas] = sum0 += (uint)(values[k] << -(int)((products[k] - (2 * k + kmin)) >> 63));
                    Debug.Assert(k == 0 || (uint)IntegerMath.NumberOfDivisors(2 * k + kmin) == sums[k - deltas] - sums[k - deltas - 1]);
                }
            }
            else if (sums == null)
            {
